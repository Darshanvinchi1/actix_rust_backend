...
use crate::{
    db::UserExt,
    dtos::{
        FilterUserDto, LoginUserDto, RegisterUserAdminDto, RegisterUserDto, UserData,
...
pub fn auth_scope() -> Scope {
    web::scope("/api/auth")
        ...
        .route("/register_admin", web::post().to(admin_register))
...
#[utoipa::path(
    ...
    tag = "Admin Register Account Endpoint",
    request_body(content = RegisterUserAdminDto, description = "Credentials to create account", example = json!({"email": "johndoe@example.com","name": "John Doe","password": "password123","passwordConfirm": "password123","role":"admin"})),
    responses(
        (status=201, description= "Account created successfully", body= UserResponseDto ),
        (status=400, description= "Validation Errors", body= Response),
        (status=409, description= "User with email already exists", body= Response),
...
pub async fn admin_register(
    app_state: web::Data<AppState>,
    body: web::Json<RegisterUserAdminDto>,
) -> Result<HttpResponse, HttpError> {
    body.validate()
        .map_err(|e| HttpError::bad_request(e.to_string()))?;
