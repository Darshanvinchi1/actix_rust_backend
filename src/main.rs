...
use dotenv::dotenv;
use dtos::{
    FilterUserDto, LoginUserDto, RegisterUserAdminDto, RegisterUserDto, Response, UserData,
...
#[openapi(
    paths(
        auth::login,auth::logout,auth::register,auth::admin_register ,users::get_me, users::get_users, health_checker_handler
    ),
    components(
        schemas(UserData,FilterUserDto,RegisterUserAdminDto,LoginUserDto,RegisterUserDto,UserResponseDto,UserLoginResponseDto,Response,UserListResponseDto)
    ),
    tags(
